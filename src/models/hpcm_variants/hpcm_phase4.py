"""
HPCM Phase 4: Spatial Prior Enhancement with RWKV
Replace y_spatial_prior DWConvRB-based networks with RWKV for better entropy estimation
"""
...
# Phase 4: y_spatial_prior classes replaced with RWKV versions
# See RWKVSpatialPrior_S1_S2 and RWKVSpatialPrior_S3 in rwkv_modules


class HPCM_Phase4(basemodel):
    """
    HPCM Phase 4: Spatial Prior Enhancement with RWKV (FINAL PHASE)
    
    Modifications (cumulative from Phase 3):
        ...
        - context_net: conv1x1 → RWKVFusionNet (num_blocks=1, hidden_rate=4)
        - y_spatial_prior_s1_s2: DWConvRB → RWKVSpatialPrior_S1_S2 [NEW]
        - y_spatial_prior_s3: DWConvRB → RWKVSpatialPrior_S3 [NEW]
        
    Expected benefits:
        - 40-55% total encoding time reduction (fully RWKV-optimized)
        - +0.3~0.55 dB performance gain (better entropy estimation)
        - Improved rate-distortion performance across all bitrates
        - More accurate spatial prior modeling
    """
    ...
    def __init__(self, M=320, N=256):
        ...
        # Phase 4: Replace with RWKV-enhanced spatial priors
        self.y_spatial_prior_s1_s2 = RWKVSpatialPrior_S1_S2(M, num_rwkv_blocks=2, hidden_rate=4)
        self.y_spatial_prior_s3 = RWKVSpatialPrior_S3(M, num_rwkv_blocks=3, hidden_rate=4)
