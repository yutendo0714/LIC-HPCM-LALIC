...
// change CHANNEL_SPLIT from (512 / 64) to (512 / 32), from https://github.com/Yaziwel/Restore-RWKV
...
template <typename F>
__global__ void biwkv4_kernel_forward(const int B, const int T, const int C,
...
template <typename F>
__global__ void biwkv4_kernel_backward(const int B, const int T, const int C,
...
void biwkv4_cuda_forward(int B, int T, int C, float *w, float *u, float *k, float *v, float *y) {
    ...
    biwkv4_kernel_forward<<<numBlocks, threadsPerBlock>>>(B, T, C, w, u, k, v, y);
}
...
void biwkv4_cuda_backward(int B, int T, int C, float *w, float *u, float *k, float *v, float *gy, float *gw, float *gu, float *gk, float *gv) {
    ...
    assert(B * C % threadsPerBlock.x == 0);
    dim3 numBlocks(B * C / threadsPerBlock.x);
    biwkv4_kernel_backward<<<numBlocks, threadsPerBlock>>>(B, T, C, w, u, k, v, gy, gw, gu, gk, gv);
}
